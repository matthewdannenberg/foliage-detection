...
### Ground Truth Labels
Observer labels come from the [USA National Phenology Network](https://www.usanpn.org/) (USA-NPN), which maintains a network of trained volunteers recording phenophase observations at fixed sites. Two phenophases are used:
...
**Expanded observer data.** The USA-NPN network provides the cleanest labels but has limited geographic coverage in Vermont specifically. Integration with PhenoCam time series (continuous RGB camera images at fixed forest sites) would provide dense, calibrated early/peak/late labels at verified deciduous forest locations.
